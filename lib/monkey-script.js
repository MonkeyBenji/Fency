...
const replaceDomStrings = (mapping, domElement = null) => {
  ...
  if (domElement instanceof Text) {
    ...
    for (const search in mapping) {
      if (domElement.textContent.includes(search)) domElement.textContent = domElement.textContent.replaceAll(search, mapping[search]);
    }
  } else {
    for (const search in mapping) {
...
const info = (...args) =>
  console.info(`%c ℹ️ %c ${args.shift()}`, "color: #fff; background-color: #4299E1; border-radius: 100%;", `color: #2B6494`, ...args);
