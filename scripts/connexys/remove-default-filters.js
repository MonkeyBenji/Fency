import(chrome.runtime.getURL("/lib/monkey-script.js")).then(async (Monkey) => {
  Monkey.waitForSelector(".query-builder-widget li.slds-tabs_default__item:not(.slds-is-active) .tab-label", 5 * 60 * 1000)
    .then((span) => {
      ...
      a.addEventListener("click", async (ev) => {
        ev.preventDefault();
        const filterTab = document.querySelector("li.slds-tabs_default__item:last-of-type");
        filterTab.click();
        ...
        picklistDate.click();
        const picklistItemAllTheTime = await Monkey.waitForSelector(".filter-widget .slds-dropdown__item:first-of-type a");
        ...
        if (document.querySelector(".dash-title .dash-tag").textContent === "[iw] Kandidaten") {
